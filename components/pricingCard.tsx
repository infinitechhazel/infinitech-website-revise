  ...
  return (
    <motion.div
      className={cn(
        ...
        isColumnHovered || isMobile || isTablet ? "mb-20 rounded-t-2xl" : "rounded-2xl",
      )}
    >
      {/* Popular Badge */}
      {plan.popular && (
        <motion.div
          ...
          className={`absolute -top-3 -translate-x-1/2 px-3 py-1 bg-gradient-to-r from-yellow-500 to-yellow-200 rounded-full text-xs font-bold text-black
            ${isColumnHovered || isMobile || isTablet ? "left-1/2" : "left-12"}`}
        >
          Most Popular
        </motion.div>
      )}
      ...
      >
        ...
        <div>
          ...
          {(isColumnHovered || isMobile || isTablet) && plan.badge && <p className="text-xs">{plan.badge}</p>}
        </div>
        ...
        <motion.div
          initial={false}
          animate={{
            height: isColumnHovered || isMobile || isTablet ? "auto" : 0,
            opacity: isColumnHovered || isMobile || isTablet ? 1 : 0,
          }}
          transition={{ duration: 0.3 }}
          className="overflow-hidden"
        >
          {(isColumnHovered || isMobile || isTablet) && (
            <>
              <div className="grid grid-cols-2 gap-y-2 border-t border-slate-200 pt-3">
                {plan.features.map((feature, idx) => (
                  <motion.div
                    ...
                    animate={{
                      opacity: isColumnHovered || isMobile || isTablet ? 1 : 0,
                      x: isColumnHovered || isMobile || isTablet ? 0 : -10,
                    }}
