...
const step1Schema = Yup.object().shape({
  ...
  industry_other: Yup.string().when("industries", {
    is: (industries: string[]) => industries.includes("Other"), // only required if "Other" is selected
...
const step2Schema = Yup.object().shape({
  ...
  current_system_other: Yup.string().when("current_systems", {
  ...
  satisfaction_level: Yup.string().required("Select your satisfaction level"),
})
...
const step5Schema = Yup.object().shape({
  ...
  system_of_interest_other: Yup.string().when("systems_of_interest", {
...
const step6Schema = Yup.object().shape({
  pain_points: Yup.string().required("Please describe your pain points"),
  ideal_system: Yup.string().required("Please describe your ideal system"),
  additional_comments: Yup.string().required("Please add additional comments"),
})
